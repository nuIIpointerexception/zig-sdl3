...
fn printItems(user_data: ?*usize, props: sdl3.properties.Group, name: [:0]const u8) void {
    ...
    var writer = std.fs.File.stdout().writerStreaming(&.{});
    writer.interface.print("Index: {d}, Name: \"{s}\", Type: {s}\n", .{
        index.*,
        name,
        printPropertyType(props.getType(name)),
    }) catch std.fs.File.stderr().writeAll("Standard writer error\n") catch {};
    index.* += 1;
}
...
pub fn main() !void {
    ...
    var writer = std.fs.File.stdout().writerStreaming(&.{});
    try writer.interface.print("Type of \"myStr\" is {s}\n", .{printPropertyType(properties.getType("myStr"))});
    try writer.interface.print("Type of \"isNotThere\" is {s}\n\n", .{printPropertyType(properties.getType("isNotThere"))});
